...
## 2026-06-04 — Phase 6 UI 信息减法 + 按钮放大

- 对照新一轮 `实拍图/`，确认主要问题已不再是 GIF 偏色，而是：
- 顶栏和标题区信息过密，拍照观感像重叠
- 歌曲区和进度区有过多小字，导致视觉噪声
- 底部传输按钮图标偏小，不够清楚
- 本轮只动显示层，不动音频链路：
- 删除顶部 `ESP32 MP3 PLAYER` 标题，避免状态栏下方再堆一层文字
- 状态栏文案压缩成简写：`Bxxx` / `Sxxx` / `Pxxxx` / `Axx`
- 歌曲区头部从 `TRACK 01 / 05` 收成右上角 `01/05`
- 去掉进度条上方 `TRACK PROGRESS` 标签，只保留百分比和进度条本体
- 歌名区域进一步减字：
- 每行字符数从 `22` 缩到 `18`
- 第一行上移，第二行弱化，减少拥挤感
- 底部按钮重新放大：
...
- 进度条和按钮整体上移，减少底部边缘拥挤
- 编译与烧录：
- `idf.py build` 通过，bin 大小 `0x71540`
- 已重新烧录 `/dev/ttyACM0`
- 备注：
- 这轮是“信息减法”和“图标放大”，核心目标是让实拍观感更干净
- `实拍图/` 仍未加入 git
